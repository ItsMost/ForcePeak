...
const CATEGORY_STYLES = {
  ...
  power: { color: 'text-yellow-500 border-yellow-350', icon: <Zap className="w-4 h-4" /> },
  strength: { color: 'text-slate-600 border-slate-350', icon: <Dumbbell className="w-4 h-4" /> },
...
export default function TimelineCard({ 
  ...
  return (
    <div 
      className={`relative flex gap-2.5 sm:gap-3 group cursor-grab active:cursor-grabbing timeline-card print-accent-${safeType} ${supersetClasses}`}
    ...
    >
      ...
      <div className="flex-1 pt-0.5 min-w-0">
        <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
          ...
          {isSuperset && (
            <span className={`text-[9px] font-black px-1.5 py-0.5 rounded tracking-wider shadow-sm shrink-0 ${sStyle.badge}`}>
              {supersetLabel}
            </span>
          )}
          ...
          {calculatedWeight && (
            <span className="px-1.5 py-0.5 text-[9px] font-black bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400 rounded shrink-0 border border-blue-200 dark:border-blue-800">
              {calculatedWeight} KG
            </span>
          )}

          {drill.percentage && (
            <span className="px-1.5 py-0.5 text-[9px] font-black bg-orange-100 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400 rounded shrink-0">
              {drill.percentage}%
            </span>
          )}
        </div>
        ...
        {(drill.sets || (isMeters ? drill.distance : drill.reps) || drill.rest) && (
          <div className="flex items-center gap-1 mt-1 mb-1.5 flex-wrap">
            {(drill.sets || (isMeters ? drill.distance : drill.reps)) && (
              <>
                <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded font-black text-[10px] tracking-wider">
                  {drill.sets || '-'} Sets
                </span>
            ...
            {drill.rest && (
              <span className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded font-black text-[10px] tracking-wider flex items-center gap-1 mr-1">
        ...
        {drill.details && (
          <p className="text-[11px] md:text-[12px] font-bold text-slate-500 dark:text-slate-400 whitespace-pre-line leading-tight mt-1">
            {drill.details}
          </p>
        )}
        ...
        {!isPreviewMode && (
          <div className="flex items-center justify-start flex-wrap gap-1 mt-3 opacity-0 group-hover:opacity-100 transition-opacity print:hidden">
