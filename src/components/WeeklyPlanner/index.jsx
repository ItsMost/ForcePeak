...
export default function WeeklyPlanner() {
    ...
    const fetchAthletes = async () => {
      ...
      if (data && data.length > 0) {
        const formattedData = data.map(a => ({ 
          ...a, 
          birthYear: a.birth_year, bodyFat: a.body_fat, verticalJump: a.vertical_jump, 
          standingLongJump: a.standing_long_jump, squatJump: a.squat_jump,
          halfSquat: a.half_squat, quarterSquat: a.quarter_squat, fullSquat: a.full_squat, deadlift: a.deadlift 
        }));
  ...
  useEffect(() => {
    const fetchMonthData = async () => {
      ...
      const startStr = getDbDateStr(startOfMonth);
      const endStr = getDbDateStr(endOfMonth);
  ...
  const handleCopyDay = (day) => { if (schedule[day].length === 0) { handleToast('No exercises to copy on this day'); return; } setClipboard({ type: 'day', data: schedule[day] }); handleToast(`${day} workouts copied`); };
  const handleCopyWeek = () => { setClipboard({ type: 'week', data: { schedule, dayTitles } }); handleToast('Full week configuration copied'); };
  ...
  const handlePasteIntoDay = (targetDay) => {
    ...
    else if (clipboard.type === 'week') { handleToast('Use side menu interface to paste a whole week block'); return; }
  ...
  const calculateDayVolume = (dayDrills) => { 
    ...
    dayDrills.forEach(drill => { 
      ...
      if (type === 'power' && (drill.unit || '').toLowerCase() === 'jumps') {
        jumpsVolume += repsMultiplier;
      }

      if (type === 'power' || type === 'strength') {
        let categoryMultiplier = 1;
        if (type === 'power') categoryMultiplier = 2.0;       
        else if (type === 'strength') categoryMultiplier = 1.5; 
        ...
        if (title.includes('recovery') || title.includes('break') || repsMultiplier === 0) { drillLoad = 0; }
  ...
  const weeklyStats = useMemo(() => {
    ...
    DAYS_OF_WEEK.forEach(day => {
      ...
      totalJumps += stats.jumpsVolume;
      totalCnsLoad += stats.cnsLoad;
    ...
    let loadLabel = 'Low (Deload)';
    let loadColor = 'text-green-600 bg-green-50 border-green-200 dark:bg-green-500/10 dark:border-green-800';
    if (totalLoad >= 9000) { loadLabel = 'Very High (Overreaching)'; loadColor = 'text-red-600 bg-red-50 border-red-200 dark:bg-red-500/10 dark:border-red-800'; } 
    else if (totalLoad >= 5000) { loadLabel = 'High (High)'; loadColor = 'text-orange-600 bg-orange-50 border-orange-200 dark:bg-orange-500/10 dark:border-orange-800'; } 
    else if (totalLoad >= 2500) { loadLabel = 'Medium (Base)'; loadColor = 'text-yellow-600 bg-yellow-50 border-yellow-200 dark:text-yellow-400 dark:bg-yellow-500/10 dark:border-yellow-800'; }
  ...
  const handleSaveProgramBlock = async () => {
    ...
    const payload = {
      ...
      weeks: compiledWeeks.map(w => ({ ...w, blockTags: createProgramModal.tags }))
    };
  ...
  const handleApplyProgramBlock = async (program) => {
    ...
    for (let i = 0; i < program.weeks.length; i++) {
      ...
      for (let j = 0; j < DAYS_OF_WEEK.length; j++) {
        ...
        const clonedDrills = (weekTemplateObject[dayName] || []).map((drill, idx) => ({
          ...drill,
  ...
  const renderLargeCalendarDays = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      let displayTitle = "";
      if (isSelectedWeek) { const dayName = JS_DAYS[dateObj.getDay()]; displayTitle = dayTitles[dayName] || (hasWorkoutSaved ? "Workout" : ""); } 
      else if (hasWorkoutSaved) { displayTitle = dayData.title || "Workout"; }
      ...
      days.push(
        <button key={i} onClick={() => { const newDate = new Date(currentDate); newDate.setDate(i); setCurrentDate(newDate); setShowMonthCalendar(false); }} 
          className={`h-20 sm:h-28 w-full rounded-xl sm:rounded-2xl p-1 sm:p-3 flex flex-col items-center sm:items-start justify-start border transition-all overflow-hidden 
            ${isSelectedWeek ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800 shadow-sm' 
            : hasWorkoutSaved ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 shadow-sm hover:bg-blue-100' 
            : 'border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
          <span className={`text-[10px] sm:text-sm font-bold w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center rounded-full mb-1 sm:mb-2 shrink-0 \
            ${isSelectedWeek ? 'bg-orange-500 text-white' : hasWorkoutSaved ? 'bg-blue-500 text-white' : 'text-slate-700 dark:text-slate-300'}`}>{i}</span>
          {(displayTitle) && ( <span className={`text-[8px] sm:text-xs font-medium text-center sm:text-left line-clamp-2 leading-tight break-words w-full ${isSelectedWeek ? 'text-orange-700 dark:text-orange-400' : 'text-blue-700 dark:text-blue-400'}`}>{displayTitle}</span> )}
        </button>
      );
    } return days;
  };
  ...
  const handleLibraryDropzone = async (e) => {
    ...
    if (!error && data) { setLibrary(prev => ({ ...prev, drills: [data[0], ...prev.drills] })); handleToast('Exercise saved to library sidebar archive!'); }
    setDraggedItem(null);
  };
  ...
  const handleDeleteLibraryTemplate = async (id) => { const { error } = await supabase.from('agilitylap_templates').delete().eq('id', id); if (!error) { setLibrary(prev => ({ ...prev, templates: prev.templates.filter(t => t.id !== id) })); handleToast('Template removed from system archive'); } };
  const handleEditTemplate = (tpl) => { handleToast('To adjust a template, drag it to any day timeline, alter it, and save it again.'); };
  
  const handleSaveLibraryExercise = async () => { 
    if(!addExerciseModal.title.trim()) { handleToast('Please enter an exercise name!'); return; } 
    ...
    if (addExerciseModal.id) {
      ...
      if(!error && data) { setLibrary(prev => ({ ...prev, drills: prev.drills.map(d => d.id === addExerciseModal.id ? data[0] : d) })); setAddExerciseModal({ isOpen: false, id: null, title: '', details: '', type: 'strength', percentage: '', sets: '', reps: '', rest: '', unit: 'reps' }); handleToast('Exercise updated successfully'); }
    } else {
      const { data, error } = await supabase.from('library_drills').insert([drillData]).select();
      if(!error && data) { setLibrary(prev => ({ ...prev, drills: [data[0], ...prev.drills] })); setAddExerciseModal({ isOpen: false, id: null, title: '', details: '', type: 'strength', percentage: '', sets: '', reps: '', rest: '', unit: 'reps' }); handleToast('Exercise added to Library database'); }
    }
  };
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-orange-500/30 transition-colors duration-200 ${isDarkMode ? 'dark bg-slate-900 text-slate-100' : 'bg-[#F4F5F7] text-slate-800'} print:bg-white print:text-black`}>
      ...
      {deleteConfirmation.isOpen && ( <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden"> <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-700 p-6 text-center"> <div className="w-16 h-16 bg-red-100 dark:bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-4"><AlertTriangle className="w-8 h-8 text-red-500" /></div> <h3 className="text-lg font-bold mb-2">Are you sure?</h3> <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">{deleteConfirmation.type === 'week' ? "This will erase all structural days exercises permanently." : "This day's workload records will be wiped out."}</p> <div className="flex gap-3"> <button onClick={() => setDeleteConfirmation({isOpen: false})} className="flex-1 px-4 py-2.5 rounded-xl text-slate-600 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 transition-colors font-medium text-sm">Cancel</button> <button onClick={confirmDelete} className="flex-1 px-4 py-2.5 bg-red-50 hover:bg-red-600 text-white rounded-xl shadow-sm transition-colors font-medium text-sm">Yes, Delete</button> </div> </div> </div> )}
      ...
      {createProgramModal.isOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md p-6 border border-slate-200 dark:border-slate-700">
            <h3 className="text-lg font-bold mb-4 text-slate-800 dark:text-white">Create Multi-Week Program Block</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">PROGRAM BLOCK NAME</label>
                <input type="text" value={createProgramModal.name} onChange={(e) => setCreateProgramModal({...createProgramModal, name: e.target.value})} placeholder="e.g., Return from Absence Block" className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-orange-500/40" />
              </div>
              
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">CUSTOM CATEGORY TAGS</label>
                <input type="text" value={createProgramModal.tags || ''} onChange={(e) => setCreateProgramModal({...createProgramModal, tags: e.target.value})} placeholder="e.g., ReturnToPlay, StrengthPhase" className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-orange-500/40" />
              </div>

              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {createProgramModal.weeksChain.map((selectedTplId, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-400 shrink-0 w-16">Week {idx + 1}:</span>
                    <select value={selectedTplId} onChange={(e) => {
                      const updated = [...createProgramModal.weeksChain]; updated[idx] = e.target.value;
                      setCreateProgramModal({...createProgramModal, weeksChain: updated});
                    }} className="flex-1 text-sm bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 p-2 rounded-xl outline-none focus:ring-2 focus:ring-orange-500/30">
                      <option value="">-- Choose Week Template --</option>
                      {library.templates.map(t => <option key={t.id} value={t.id}>{t.title}</option>)}
                    </select>
                  </div>
                ))}
              </div>
              <button onClick={() => setCreateProgramModal({...createProgramModal, weeksChain: [...createProgramModal.weeksChain, '']})} className="text-xs font-bold text-orange-500 hover:text-orange-600 flex items-center gap-1">+ Add Future Week Link</button>
            </div>
            <div className="flex justify-end gap-2 mt-6">
              <button onClick={() => setCreateProgramModal({ isOpen: false, name: '', tags: '', weeksChain: [''] })} className="px-4 py-2 text-sm font-bold text-slate-500 hover:bg-slate-100 rounded-xl">Cancel</button>
              <button onClick={handleSaveProgramBlock} className="px-5 py-2 text-sm font-bold bg-orange-500 text-white rounded-xl shadow-md hover:bg-orange-600">Save Block</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-700">
            <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-slate-700">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                ...
                {dayDrillModal.isNew ? 'Add New Exercise' : 'Edit Performance Metrics'}
              </h3>
            ...
            <div className="p-5 space-y-4">
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <select value={dayDrillModal.drill.type} onChange={(e) => setDayDrillModal({...dayDrillModal, drill: {...dayDrillModal.drill, type: e.target.value}})} className="w-full text-sm px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
              ...
              <div className="flex gap-2 sm:gap-3">
                <div className="flex-1">
                  ...
                  <input type="text" value={dayDrillModal.drill.sets || ''} onChange={(e) => setDayDrillModal({...dayDrillModal, drill: {...dayDrillModal.drill, sets: e.target.value}})} placeholder="e.g. 3" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white font-medium text-sm" />
                </div>
                <div className="flex-1">
                  <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase">Volume</label>
                  <input type="text" value={dayDrillModal.drill.reps || ''} onChange={(e) => setDayDrillModal({...dayDrillModal, drill: {...dayDrillModal.drill, reps: e.target.value}})} placeholder="Value" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white font-medium text-sm" />
                </div>
                
                <div className="w-24">
                  <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase">Unit</label>
                  <select value={dayDrillModal.drill.unit || 'reps'} onChange={(e) => setDayDrillModal({...dayDrillModal, drill: {...dayDrillModal.drill, unit: e.target.value}})} className="w-full text-sm px-2 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                ...
                <div className="flex-1">
                  <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase">Rest</label>
                  <input type="text" value={dayDrillModal.drill.rest || ''} onChange={(e) => setDayDrillModal({...dayDrillModal, drill: {...dayDrillModal.drill, rest: e.target.value}})} placeholder="90s" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white font-medium text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase">Exercise Title Name</label>
                <input type="text" value={dayDrillModal.drill.title} onChange={(e) => setDayDrillModal({...dayDrillModal, drill: {...dayDrillModal.drill, title: e.target.value}})} placeholder="Exercise Name..." className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white font-medium" autoFocus />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase">Specific Coaching Notes</label>
                <textarea value={dayDrillModal.drill.details} onChange={(e) => setDayDrillModal({...dayDrillModal, drill: {...dayDrillModal.drill, details: e.target.value}})} placeholder="Coaching cues or alternative structural targets..." className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white h-20 resize-none" />
              </div>
            </div>
            <div className="p-5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button onClick={() => setDayDrillModal({ isOpen: false, day: null, drill: null, isNew: false })} className="px-4 py-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors font-bold text-sm">Cancel</button>
                <button onClick={handleSaveDayDrillModal} className={`px-6 py-2 ${dayDrillModal.isNew ? 'bg-green-500 hover:bg-green-600' : 'bg-blue-500 hover:bg-blue-600'} text-white rounded-xl shadow-md hover:shadow-lg transition-all font-bold text-sm flex items-center gap-2`}><Save className="w-4 h-4"/> {dayDrillModal.isNew ? 'Add' : 'Save'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Analytics Summary Dashboard Modal (Barchart) with integrated CNS split analytics */}
      {showStatsModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4 print:hidden" onClick={() => setShowStatsModal(false)}>
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-200 dark:border-slate-700" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-5 md:p-6 border-b border-slate-100 dark:border-slate-700">
              <h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
                <BarChart3 className="w-6 h-6 text-orange-500" /> Weekly Workload Analytics
              </h3>
              <button onClick={() => setShowStatsModal(false)} className="p-2 bg-slate-100 dark:bg-slate-700 text-slate-500 hover:text-slate-800 dark:hover:text-white rounded-full transition-colors"><X className="w-5 h-5"/></button>
            </div>
            <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className={`p-4 rounded-2xl border ${weeklyStats.loadColor}`}>
                  <p className="text-xs font-bold uppercase tracking-widest opacity-70 mb-1">Total Weekly Load</p>
                  <p className="text-3xl font-black">{weeklyStats.load} <span className="text-sm font-medium opacity-80">AU</span></p>
                  <p className="text-sm font-bold mt-1 opacity-90">Status: {weeklyStats.loadLabel}</p>
                </div>
                <div className="p-4 rounded-2xl border border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-400">
                  <p className="text-xs font-bold uppercase tracking-widest opacity-70 mb-1">Average Intensity</p>
                  <p className="text-3xl font-black">{weeklyStats.intensity}%</p>
                </div>
              </div>

              {/* Weekly Performance Counters Dashboard Panel */}
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-xl text-center">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Total Jumps</p>
                  <p className="text-xl font-black text-orange-500 mt-0.5">{weeklyStats.totalJumps}</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-xl text-center">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">CNS Split</p>
                  <p className="text-xl font-black text-yellow-500 mt-0.5">{weeklyStats.cnsPercentage}%</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-xl text-center">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Structural</p>
                  <p className="text-xl font-black text-blue-500 mt-0.5">{weeklyStats.structuralPercentage}%</p>
                </div>
              </div>

              {/* Macro Weekly CNS vs Structural Ratio Bar */}
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-bold text-slate-500">
                  <span>CNS / Power Fatigue Ratio</span>
                  <span>Structural Muscle Strain</span>
                </div>
                <div className="w-full h-4 bg-slate-100 dark:bg-slate-700 rounded-full flex overflow-hidden border border-slate-200/50 dark:border-slate-600/50">
                  <div className="h-full bg-gradient-to-r from-amber-500 to-orange-500 transition-all duration-500" style={{ width: `${weeklyStats.cnsPercentage}%` }} />
                  <div className="h-full bg-gradient-to-r from-blue-500 to-indigo-600 transition-all duration-500" style={{ width: `${weeklyStats.structuralPercentage}%` }} />
                </div>
                <div className="flex justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <span>{weeklyStats.cnsPercentage}% CNS</span>
                  <span>{weeklyStats.structuralPercentage}% Structural</span>
                </div>
              </div>

              <div>
                <h4 className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-4 uppercase tracking-widest">Daily Load Distribution</h4>
                <div className="flex items-end justify-between gap-2 h-36 border-b border-slate-200 dark:border-slate-700 pb-2">
                  {weeklyStats.dailyData.map((data, i) => {
                    ...
                    return (
                      <div key={i} className="flex flex-col items-center flex-1 group h-full justify-end">
                        <div className="text-[9px] font-bold text-slate-400 mb-1 opacity-0 group-hover:opacity-100 transition-opacity">{data.load}</div>
                        <div className="w-full max-w-[40px] bg-slate-100 dark:bg-slate-700 rounded-t-md relative flex items-end justify-center overflow-hidden h-full mt-1">
                          <div className="w-full bg-orange-500 rounded-t-md transition-all duration-700 ease-out" style={{ height: `${heightPercent}%`, minHeight: data.load > 0 ? '4px' : '0' }}></div>
                        </div>
        ...
        <div className={`flex-1 overflow-x-auto overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50 print:bg-white print:overflow-visible w-full pb-24 md:pb-0 relative transition-all duration-300 ${showLibrary ? 'md:mr-80' : ''}`}>
          ...
          {isLoading && (
             <div className="absolute inset-0 flex items-center justify-center bg-white/90 dark:bg-slate-900/90 backdrop-blur-md z-50 transition-opacity duration-300 print:hidden">
                <div className="flex flex-col items-center gap-4">
                   ...
                   <p className="text-slate-600 dark:text-slate-300 font-bold text-lg animate-pulse">Loading Training Blocks...</p>
                </div>
             </div>
          )}
          ...
          <div className={`flex h-full p-2 md:p-4 gap-2 print:grid print:grid-cols-2 print:gap-x-12 print:gap-y-6 print:p-4 ${isMobileView ? 'flex-col w-full min-h-max' : 'flex-row w-full'}`}>
            {DAYS_OF_WEEK.map((day, index) => {
              ...
              const dayTotalCombined = dayStats.cnsLoad + dayStats.structuralLoad;
              const dayCnsPct = dayTotalCombined > 0 ? Math.round((dayStats.cnsLoad / dayTotalCombined) * 100) : 0;
                  ...
                  <div className="flex items-start gap-1 md:gap-2 justify-between">
                    <div className="flex items-start gap-1 md:gap-2 flex-1">
                      ...
                      <input 
                         ...
                         placeholder="Add Workout Focus" className="text-xs md:text-[14px] font-medium text-slate-700 dark:text-slate-200 bg-transparent border-none outline-none placeholder-slate-400 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 rounded px-1 -ml-1 transition-colors w-full focus:ring-2 focus:ring-orange-500/50 print:placeholder-transparent print:text-black"
                         readOnly={isPreviewMode}
                      />
                    </div>
                    {!isPreviewMode && (
                      <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity print:hidden">
                         <button onClick={() => handleCopyDay(day)} className="p-1 text-slate-400 hover:text-blue-500" title="Copy Daily Blocks"><Copy className="w-3 h-3 md:w-4 md:h-4" /></button>
                         {clipboard && ( <button onClick={() => handlePasteIntoDay(day)} className="p-1 text-slate-400 hover:text-green-500" title="Paste Block"><ClipboardPaste className="w-3 h-3 md:w-4 md:h-4" /></button> )}
                         <button onClick={() => setSaveTemplateModal({isOpen: true, day, name: dayTitles[day] || ''})} className="p-1 text-slate-400 hover:text-orange-500" title="Save Routine Template"><BookmarkPlus className="w-3 h-3 md:w-4 md:h-4" /></button>
                  ...
                  {schedule[day].length > 0 && !isPreviewMode && (
                    <div className="mt-4 p-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-2 print:hidden">
                      <div className="flex justify-between items-center text-[10px] sm:text-xs font-bold">
                        <div className="flex flex-col items-center text-slate-400" title="Total Stations">
                          <span className="text-[8px] uppercase">Drills</span>
                          <span className="text-slate-700 dark:text-slate-300">{dayStats.totalExercises}</span>
                        </div>
                        <div className="w-px h-5 bg-slate-200 dark:bg-slate-700"></div>
                        <div className="flex flex-col items-center text-blue-500" title="Average Intensity">
                          <span className="text-blue-400 text-[8px] uppercase">Intensity</span>
                          <span>{dayStats.avgIntensity}%</span>
                        </div>
                        <div className="w-px h-5 bg-slate-200 dark:bg-slate-700"></div>
                        <div className="flex flex-col items-center text-orange-500" title="Strength + Power Absolute Load Score">
                          <span className="text-orange-400 text-[8px] uppercase">Load</span>
                          <span>{dayStats.totalVolumeScore}</span>
                        </div>
                        {dayStats.jumpsVolume > 0 && (
                          <>
                            <div className="w-px h-5 bg-slate-200 dark:bg-slate-700"></div>
                            <div className="flex flex-col items-center text-amber-500" title="Total Jumps Reps Count">
                              <span className="text-amber-400 text-[8px] uppercase">Jumps</span>
                              <span>{dayStats.jumpsVolume}</span>
                            </div>
                          </>
                        )}
                      </div>
                      ...
                      {dayTotalCombined > 0 && (
                        <div className="space-y-0.5">
                          <div className="w-full h-1 bg-slate-100 dark:bg-slate-700 rounded-full flex overflow-hidden">
                            <div className="h-full bg-amber-400" style={{ width: `${dayCnsPct}%` }} title={`CNS Power: ${dayCnsPct}%`} />
                            <div className="h-full bg-blue-500" style={{ width: `${100 - dayCnsPct}%` }} title={`Structural: ${100 - dayCnsPct}%`} />
                          </div>
                        </div>
                      )}
                    </div>
