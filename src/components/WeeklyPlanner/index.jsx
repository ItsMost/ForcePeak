...
export default function WeeklyPlanner() {
  ...
  const renderLargeCalendarDays = () => {
    ...
    for (let i = 0; i < startDay; i++)
      days.push(
        <div key={`empty-${i}`} className="p-4 border border-transparent"></div>
      );
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <button
          ...
          className={`h-28 w-full rounded-2xl p-3 flex flex-col items-start justify-start border transition-all ${
            isSelectedWeek
        ...
        >
          ...
          {isSelectedWeek && (
            <span className="text-xs font-medium text-slate-600 dark:text-slate-300 text-left line-clamp-2">
              {dayTitle}
            </span>
          )}
        </button>
  ...
  const handleDragStart = (e, day, drill, index) => {
  ...
  const handleDrop = (e, targetDay, targetIndex = null) => {
    ...
    setSchedule((prev) => {
      ...
      if (draggedItem.source === 'timeline') {
        ...
        newSchedule[sourceDay] = [...newSchedule[sourceDay]];
        ...
        if (targetIndex !== null) {
          ...
          if (sourceDay === targetDay && sourceIndex < targetIndex)
            adjustedIndex -= 1;
          newSchedule[targetDay].splice(adjustedIndex, 0, drill);
        } else {
          newSchedule[targetDay].push(drill);
        }
  ...
  const handleUpdateExercise = (day, id, updatedDrill) => {
    setSchedule((prev) => ({
      ...prev,
      [day]: prev[day].map((w) => (w.id === id ? updatedDrill : w)),
    }));
  };
  const handleDeleteExercise = (day, id) => {
    setSchedule((prev) => ({
      ...prev,
      [day]: prev[day].filter((w) => w.id !== id),
    }));
  };
  const handleDayTitleChange = (day, newTitle) =>
    setDayTitles((prev) => ({ ...prev, [day]: newTitle }));
  ...
  const confirmDelete = () => {
    if (deleteConfirmation.type === 'week') {
      ...
      setDayTitles({});
      handleToast('تم تفريغ الأسبوع بالكامل');
    } else if (
      deleteConfirmation.type === 'day' &&
      deleteConfirmation.targetDay
    ) {
      setSchedule((prev) => ({ ...prev, [deleteConfirmation.targetDay]: [] }));
      const newTitles = { ...dayTitles };
      delete newTitles[deleteConfirmation.targetDay];
      setDayTitles(newTitles);
      handleToast(`تم تفريغ تمارين يوم ${deleteConfirmation.targetDay}`);
    }
  ...
  const handleSaveTemplate = () => {
    ...
    handleToast(`تم حفظ القالب: ${newTemplate.title}`);
  };
  const handleSaveWeekTemplate = () => {
    ...
    if (allDrills.length === 0) {
      handleToast('الأسبوع فارغ، لا يوجد شيء لحفظه');
      return;
    }
    const newTemplate = {
      id: `tpl-week-${Date.now()}`,
      title: saveWeekTemplateModal.name,
      type: 'mixed',
      drills: allDrills,
    };
    setLibrary((prev) => ({
    ...
    handleToast(`تم حفظ الأسبوع كقالب: ${newTemplate.title}`);
  };
  const handleSaveLibraryExercise = () => {
    if (!addExerciseModal.title.trim()) {
      handleToast('يرجى كتابة اسم التمرين!');
      return;
    }
    const newExercise = {
    ...
    setLibrary((prev) => ({ ...prev, drills: [newExercise, ...prev.drills] }));
    setAddExerciseModal({
      isOpen: false,
      title: '',
      details: '',
  ...
  const handleAddAthlete = () => {
    if (newAthleteData.name.trim()) {
      ...
      const newAthlete = {
        ...
        birthYear: newAthleteData.birthYear || null,
        height: null,
        weight: newAthleteData.weight || null,
        bodyFat: null,
        verticalJump: null,
        clean: null,
        halfSquat: null,
      ...
      handleToast(`تمت إضافة: ${newAthlete.name}`);
    }
  };
  ...
  const handleExportPDF = () => {
    ...
    handleToast('يتم التجهيز للطباعة/PDF...');
    setTimeout(() => {
      window.print();
    }, 500);
  };
  ...
  const handleExportExcel = () => {
    ...
    csvContent += 'Day,Exercise Title,Type,Percentage,Details\n';
    DAYS_OF_WEEK.forEach((day) => {
      schedule[day].forEach((drill) => {
        ...
        csvContent += `${day},${title},${type},${percentage},${details}\n`;
      });
    });
    ...
    link.setAttribute(
      'download',
      `Weekly_Plan_${selectedAthlete.name.replace(' ', '_')}.csv`
    );
  ...
  const calculateDayVolume = (dayDrills) => {
    ...
    dayDrills.forEach((drill) => {
      ...
      if (match) {
        repsMultiplier = parseInt(match[1]) * parseInt(match[2]);
      }
      ...
      totalVolumeScore +=
        repsMultiplier * (intensity > 0 ? intensity / 100 : 1);
    });
    const avgIntensity =
    ...
    return {
      totalExercises,
      totalVolumeScore: Math.round(totalVolumeScore * 10),
      avgIntensity,
    };
  };
  ...
  return (
    ...
    >
      ...
      {showMonthCalendar && (
        <div
          className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden"
          onClick={() => setShowMonthCalendar(false)}
        >
          <div
            className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl p-8 w-full max-w-4xl border border-slate-200 dark:border-slate-700"
          ...
          >
            <div className="flex justify-between items-center mb-8">
              ...
              <button
                onClick={() => setShowMonthCalendar(false)}
                className="p-2 bg-slate-100 dark:bg-slate-700 text-slate-500 hover:text-slate-800 dark:hover:text-white rounded-full transition-colors"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-200 dark:border-slate-700 p-6 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
              {deleteConfirmation.type === 'week'
                ? 'سيتم مسح جميع التمارين في هذا الأسبوع بشكل نهائي.'
                : `سيتم مسح جميع تمارين يوم ${deleteConfirmation.targetDay}.`}
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteConfirmation({ isOpen: false })}
                className="flex-1 px-4 py-2.5 rounded-xl text-slate-600 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 transition-colors font-medium text-sm"
              >
                إلغاء
              </button>
              <button
                onClick={confirmDelete}
                className="flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl shadow-sm transition-colors font-medium text-sm"
              >
                نعم، امسح
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-700 p-6">
            <h3 className="text-lg font-bold mb-2 text-slate-800 dark:text-white flex items-center gap-2">
              <BookmarkPlus className="w-5 h-5 text-orange-500" /> حفظ قالب
              لليوم
            </h3>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all dark:text-white mb-6"
              autoFocus
            />
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors font-medium text-sm"
              >
                إلغاء
              </button>
              <button
                onClick={handleSaveTemplate}
                className="px-6 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl shadow-sm transition-colors font-medium text-sm"
              >
                حفظ بالمكتبة
              </button>
            </div>
      ...
      {saveWeekTemplateModal.isOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-700 p-6">
            <h3 className="text-lg font-bold mb-2 text-slate-800 dark:text-white flex items-center gap-2">
              <BookmarkPlus className="w-5 h-5 text-orange-500" /> حفظ الأسبوع
              كقالب (Template)
            </h3>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
              اسم البرنامج
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-700 p-6">
            <h3 className="text-lg font-bold mb-4 text-slate-800 dark:text-white flex items-center gap-2">
              <Plus className="w-5 h-5 text-orange-500" /> إضافة تمرين للمكتبة
            </h3>
            <div className="space-y-4">
              <div className="flex gap-2">
                <div className="flex-1">
                  ...
                  <select
                    ...
                    className="w-full text-sm p-2 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:border-orange-500"
                  >
                ...
                <div className="w-24">
                  ...
                  <div className="relative w-full">
                    <input
                      ...
                      className="w-full text-sm p-2 pl-7 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:border-orange-500"
                      placeholder="0"
                    />
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:border-orange-500 transition-all dark:text-white"
                  autoFocus
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  placeholder="5 sets x 5 reps..."
                  className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 focus:outline-none focus:border-orange-500 transition-all dark:text-white h-20 resize-none"
                />
              </div>
            </div>
            <div className="flex justify-end gap-3 mt-6">
              <button
                onClick={() =>
                  setAddExerciseModal({ ...addExerciseModal, isOpen: false })
                }
                className="px-4 py-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors font-medium text-sm"
              >
                إلغاء
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4 print:hidden">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-700 p-6">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
              <UserPlus className="w-5 h-5 text-orange-500" /> Add New Athlete
            </h3>
            <div className="space-y-3 mb-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 focus:ring-2 focus:ring-orange-500 dark:text-white"
                  autoFocus
                />
              </div>
              <div className="flex gap-3">
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors font-medium"
              >
                Cancel
              </button>
              <button
                onClick={handleAddAthlete}
                className="px-6 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl shadow-sm transition-colors font-medium"
              >
                Add Athlete
              </button>
            </div>
      ...
      <div className="flex transition-all duration-300 mx-auto h-[calc(100vh-64px)] overflow-hidden relative print:h-auto print:overflow-visible">
        <Sidebar
          isMobileView={isMobileView}
          isPreviewMode={isPreviewMode}
          setIsPreviewMode={setIsPreviewMode}
          onCopy={() => handleToast('تم نسخ الجدول')}
          onClearWeek={() =>
            setDeleteConfirmation({ isOpen: true, type: 'week' })
          }
          onShare={() => handleToast('تم النسخ للمشاركة')}
        ...
        <div className="flex-1 overflow-x-auto overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50 print:bg-white print:overflow-visible w-full">
          ...
          <div
            className={`flex h-full p-4 gap-2 print:grid print:grid-cols-2 print:gap-x-12 print:gap-y-6 print:p-4 ${
              isMobileView ? 'flex-col w-full' : 'min-w-[1200px]'
            }`}
          >
            {DAYS_OF_WEEK.map((day, index) => (
              <div
                ...
                className={`flex flex-col ${
                  isMobileView
                    ? 'w-full mb-6 border-b border-slate-200 dark:border-slate-700 pb-6'
                    : 'flex-1 min-w-[220px]'
              ...
              >
                <div className="mb-4 flex flex-col group border-b border-slate-200 dark:border-slate-700 pb-3 px-2 print:border-slate-400">
                  <span className="text-xs font-semibold tracking-wider text-slate-400 uppercase mb-2 print:text-slate-600">
                    {day}
                  </span>
                    ...
                    {!isPreviewMode && (
                      <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity print:hidden">
                        <button
                          ...
                          title="حفظ هذا اليوم كقالب"
                        >
                          <BookmarkPlus className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() =>
                            handleToast('اقتراحات الذكاء الاصطناعي تم تطبيقها')
                          }
                          className="p-1 text-orange-400 hover:text-orange-500"
                        >
                ...
                <div
                  className={`flex-1 px-2 pb-20 ${
                    draggedItem && draggedItem.sourceDay !== day
                ...
                >
                  {schedule[day].map((drill, drillIndex) => (
                    <TimelineCard
                      ...
                      day={day}
                      index={drillIndex}
                      isLast={drillIndex === schedule[day].length - 1}
                      isPreviewMode={isPreviewMode}
                  ...
                  {!isPreviewMode && (
                    <div
                      className="flex items-center gap-2 mt-2 group cursor-pointer print:hidden"
                    ...
                    >
                      ...
                      <span className="text-[13px] font-medium text-slate-500 dark:text-slate-400 group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">
                        Add Exercise
                      </span>
                    </div>
                  )}
                  ...
                  {schedule[day].length > 0 && !isPreviewMode && (
                    <div className="mt-4 p-2.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex justify-between items-center text-[10px] sm:text-xs font-medium print:hidden">
                      ...
                      >
                        ...
                        <span>
                          {calculateDayVolume(schedule[day]).totalExercises}
                        </span>
                      </div>
                      <div className="w-px h-6 bg-slate-200 dark:bg-slate-700"></div>
                      <div
                        className="flex flex-col items-center text-blue-500"
                        title="متوسط الشدة المئوية"
                      >
                        <span className="text-blue-400/70 text-[9px] uppercase">
                          Avg Int
                        </span>
                        <span>
                          {calculateDayVolume(schedule[day]).avgIntensity}%
                        </span>
                      </div>
                      <div className="w-px h-6 bg-slate-200 dark:bg-slate-700"></div>
                      <div
                        className="flex flex-col items-center text-orange-500"
                        title="مؤشر الحمل التدريبي التقريبي"
                      >
                        <span className="text-orange-400/70 text-[9px] uppercase">
                          Load
                        </span>
                        <span>
                          {calculateDayVolume(schedule[day]).totalVolumeScore}
                        </span>
                      </div>
                    </div>
                  )}
